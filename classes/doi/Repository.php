...
abstract class Repository
{
    ...
    /**
     * Checks whether a DOI object is referenced by ID on any pub objects
     */
    public function isAssigned(int $doiId): bool
    {
        $publicationCount = Repo::publication()
            ->getCollector()
            ->filterByDoiIds([$doiId])
            ->getIds()
            ->count();

        $galleyCount = Repo::galley()
            ->getCollector()
            ->filterByDoiIds([$doiId])
            ->getIds()
            ->count();

        return $publicationCount || $galleyCount;
    }
